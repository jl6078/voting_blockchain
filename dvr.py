...
class NetworkInterface():
    ...
    def listen_for_messages(self, blockchain, nodeWrapper):
        depth = 0
        while True:
            try:
                data = self.recv(4096)
                ...
                message_dict = data.decode().strip()
                if message_dict["type"] == "new_neighbor":
                    # Add new neighbor to the node's neighbors list
                    nodeWrapper.neighbors.append(message_dict["neighbor_id"])
                    print(f"New neighbor added: {message_dict['neighbor_id']}")
                elif message_dict["type"] == "new_block":
                    block = Block(depth, blockchain.get_latest_block().hash, time.time(), self.message_dict["transactions"], nodeWrapper.neighbors)
...
if __name__ == '__main__':
    ...
    nodeWrapper = Node(node_id)

    nodeWrapper.neighbors = neighbors

    # Start the listener thread
    listener_thread = threading.Thread(target=net_interface.listen_for_messages, args=(blockchain, nodeWrapper,), daemon=True)
